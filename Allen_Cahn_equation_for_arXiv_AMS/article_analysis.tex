...
(b) unconditional convergence and bound-preserving property for the ADMM solver.
...
We address this by first proving its unconditional convergence, the foremost requirement for a robust solver. 
The following lemma states the existence and uniqueness of the stationary point of the Lagrange function $\mathcal{L}$. 
\begin{lemma}
...
\begin{equation}
    (e_1^{(k)},e_2^{(k)},e_3^{(k)})=(u_1^{(k)}-u_1^*,u_2^{(k)}-u_2^*,u_3^{(k)}-u_3^*),\qquad k\in \mathbb{Z},
\end{equation}
and the sequence $\{\Phi^{(k)}\}$ as
\begin{equation}\label{def:Phi}
    \Phi^{(k)}=\frac{1}{\alpha\rho}\|e_3^{(k)}\|_h^2+\rho\|e_2^{(k)}\|_h^2+
...
\begin{aligned}
    \Phi^{(k)}-\Phi^{(k+1)}\geq 
    &\alpha c(\alpha)\rho\|e_2^{(k+1)}-e_2^{(k)}\|_h^2\\+
...
\begin{equation}
    \begin{aligned}
    &\Phi^{(k)}-\Phi^{(k+1)}\\
    =&\frac{1}{\alpha \rho}(\|e_3^{(k)}\|_h^2-\|e_3^{(k+1)}\|_h^2)+\rho (\|e_2^{(k)}\|_h^2-\|e_2^{(k+1)}\|_h^2)\\
    &+\eta \rho (\|e_1^{(k)}-
    e_2^{(k)}\|_h^2-\|e_1^{(k+1)}-
    e_2^{(k+1)}\|_h^2)\\
    =&(-2\langle e_3^{(k)},e_1^{(k+1)}-e_2^{(k+1)}\rangle_h-\alpha \rho \|e_1^{(k+1)}-e_2^{(k+1)}\|_h^2)\\
    &+\rho (\|e_2^{(k+1)}-e_2^{(k)}\|_h^2
    +2\langle e_2^{(k)}-e_2^{(k+1)},e_2^{(k+1)}\rangle_h
    )\\
    &+\eta \rho (\|e_1^{(k)}-
    e_2^{(k)}\|_h^2-\|e_1^{(k+1)}-
    e_2^{(k+1)}\|_h^2)
...
\begin{subequations}
    \begin{align}
        ...
        +\rho(e_1^{(k+1)}-e_2^{(k)})+e_3^{(k)}=0,
        \label{eq:e1}
        \\
        &g(u_2^{(k+1)})-g(u_2^*)-\rho(e_1^{(k+1)}-e_2^{(k+1)})-e_3^{(k)}=0,
        \label{eq:e2}
        \\
...
\begin{equation}
    \langle (\frac{1}{\tau}-\epsilon^2\Delta_h)e_1^{(k+1)}
        +\rho(e_1^{(k+1)}-e_2^{(k)})+e_3^{(k)}, e_1^{(k+1)} \rangle_h=0,
\end{equation}
...
\begin{equation}\label{eq:inequality1}
    ...
    \leq
    -\rho \langle e_1^{(k+1)}-e_2^{(k)},e_1^{(k+1)}\rangle_h.
\end{equation}
\eqref{eq:e2} implies
\begin{equation}
    \langle g(u_2^{(k+1)})-g(u_2^*)-\rho(e_1^{(k+1)}-e_2^{(k+1)})-e_3^{(k)},u_2^{(k+1)}-u_2^* \rangle_h=0
\end{equation}
The monotonicity property of $g(u)$ yields
\begin{equation}
    \langle g(u_2^{(k+1)})-g(u_2^*),e_2^{(k+1)} \rangle_h=
    \langle \rho(e_1^{(k+1)}-e_2^{(k+1)})+e_3^{(k)},e_2^{(k+1)} \rangle_h\geq 0,
\end{equation}
which reduces to 
\begin{equation}\label{eq:inequality2}
    \langle e_3^{(k)},e_2^{(k+1)} \rangle _h\geq -\rho 
    \langle e_1^{(k+1)}-e_2^{(k+1)},e_2^{(k+1)} \rangle_h
\end{equation}
It follows from \eqref{eq:inequality1} and \eqref{eq:inequality2} that
\begin{equation}
    -2\langle e_3^{(k)},e_1^{(k+1)}-e_2^{(k+1)}\rangle_h\geq 2\rho \langle e_1^{(k+1)}-e_2^{(k)},e_1^{(k+1)}\rangle_h - 2\rho
    \langle e_1^{(k+1)}-e_2^{(k+1)},e_2^{(k+1)} \rangle_h,
\end{equation}
which leads to
\begin{equation}
...
&\Phi^{(k)}-\Phi^{(k+1)}\\
\geq &
2\rho \langle e_1^{(k+1)}-e_2^{(k)},e_1^{(k+1)}\rangle_h - 2\rho
    \langle e_1^{(k+1)}-e_2^{(k+1)},e_2^{(k+1)} \rangle_h \\
...
=&(2-\alpha-\eta)\rho \|e_1^{(k+1)}-
    e_2^{(k+1)}\|_h^2 +
    2\rho \langle e_1^{(k+1)}-e_2^{(k+1)},e_2^{(k+1)}-e_2^{(k)} \rangle_h\\
&+\rho \|e_2^{(k+1)}-e_2^{(k)}\|_h^2+
\eta \rho \|e_1^{(k)}-
    e_2^{(k)}\|_h^2
\end{aligned}
\end{equation}
Taking $k$-th and $k+1$-th in \eqref{eq:e2}, we obtain that
\begin{equation}
    g(u_2^{(k+1)})-g(u_2^{(k)})-(e_3^{(k)}-e_3^{(k-1)})-\rho[(e_1^{(k+1)}-e_2^{(k+1)})-(e_1^{(k)}-e_2^{(k)})]=0,
\end{equation}
We take the inner product with $u_2^{(k+1)}-u_2^{(k)}=e_2^{(k+1)}-e_2^{(k)}$. Then, by exploiting the monotonicity of $g(u)$ and the definition of $e_3^{(k)}$, we infer the following useful inequality that
\begin{equation}\label{eq:inequality4}
    \langle e_1^{(k+1)}-e_2^{(k+1)},e_2^{(k+1)}-e_2^{(k)}\rangle_h
    \geq(1-\alpha) \langle e_1^{(k)}-e_2^{(k)},
    e_2^{(k+1)}-e_2^{(k)}\rangle_h,
\end{equation}
which implies
\begin{align}
    &\Phi^{(k)}-\Phi^{(k+1)}\\
\geq & (2-\alpha-\eta)\rho \|e_1^{(k+1)}-
    ...
    e_2^{(k)}\|_h^2
\end{align}
If $\alpha \in (0,1]$, $\eta=1-\alpha$. From the inequality $2ab\geq -a^2-b^2$, we derive that
...
\geq & (2-\alpha-(1-\alpha))\rho \|e_1^{(k+1)}-
    ...
    +(1-\alpha) \rho \|e_1^{(k)}-
    e_2^{(k)}\|_h^2\\
    =&\alpha \rho \|e_2^{(k+1)}-e_2^{(k)}\|_h^2+\rho \|e_1^{(k+1)}-
    e_2^{(k+1)}\|_h^2.
\end{aligned}
\end{equation}
If $\alpha \in (1,+\infty)$, $\eta=1-\alpha^{-1}$. It follows from the inequality $-2ab\geq -\alpha^{-1}a^2-\alpha b^2$ that
\begin{equation}\label{eq:a>1}
\begin{aligned}
&\Phi^{(k)}-\Phi^{(k+1)}\\
\geq &(2-\alpha-(1-\alpha^{-1}))\rho \|e_1^{(k+1)}-
    ...
    \alpha \|e_2^{(k+1)}-e_2^{(k)}\|_h^2)\\
    &+\rho \|e_2^{(k+1)}-e_2^{(k)}\|_h^2
    +(1-\alpha^{-1}) \rho \|e_1^{(k)}-
    e_2^{(k)}\|_h^2\\
    =&(1+\alpha-\alpha^2)\rho \|e_2^{(k+1)}-e_2^{(k)}\|_h^2+(1-\alpha+\alpha^{-1})\rho \|e_1^{(k+1)}-
    e_2^{(k+1)}\|_h^2.
\end{aligned}
\end{equation}
The proof is completed by the above two inequalities \eqref{eq:a<1} and \eqref{eq:a>1}.
\end{proof}
Armed with the lemma above \ref{lemma:Phi}, we proceed to prove convergence.
...
\begin{proof}
    ...
    are bounded. Then, the sequences
      ...
      \begin{equation}
          \lim_{k\to +\infty}\|e_3^{(k)}\|_h=\lim_{k\to +\infty}\|e_2^{(k)}\|_h=\lim_{k\to +\infty}\|e_1^{(k)}-e_2^{(k)}\|_h=0.
      \end{equation}
      The triangle inequality \begin{equation}
          \|e_1^{(k)}\|_h\leq \|e_1^{(k)}-e_2^{(k)}\|_h+\|e_2^{(k)}\|_h 
      \end{equation}
      implies 
      \begin{equation}
          \lim_{k\to +\infty}\|e_1^{(k)}\|_h=0. 
      \end{equation}
      We have thus completed the proof.
\end{proof}
\begin{remark}
    To characterize the efficiency of our solver, we examine its asymptotic convergence rate. 
    ...
    hence it falls into the scenario $2$ of problems analyzed in \cite{deng2016global}. 
    According to Theorem 3.5 therein, our iterative solver \Cref{alg:ADMM} consequently achieves a linear convergence rate.
\end{remark}
\subsection{Bound-preserving property}
Finally, we briefly discuss the bound-preserving property of the algorithm. 
For the update of $u_2$, the presence of the logarithmic term ensures $u_2^{(k)}\in(0,1)$ at a point-wise level for all $k \in \mathbb{Z}$. 
