...
\begin{subequations}
    \begin{align}
        \label{eq:subp1}
        &u_1^{(k+1)}=\arg \min_{u_1\in \ch} \mathcal{L}_{\rho}(u_1,u_2^{(k)};u_3^{(k)}),\\
        \label{eq:subp2}
        &u_2^{(k+1)}=\arg \min_{u_2\in \ch} \mathcal{L}_{\rho}(u_1^{(k+1)},u_2;u_3^{(k)}),\\
...
\par The $u_1$-update results in a symmetric positive-definite linear system, i.e., 
    \begin{equation}
    \label{eq:u1}
        (\frac{1}{\tau}-\epsilon^2\Delta_h+\rho)u_1^{(k+1)}=\frac{1}{\tau}u^n+\rho u_2^{(k)}-u_3^{(k)},
    \end{equation}
...
\begin{equation}
    ...
    \theta(1-2u^n)-u_3^{(k)}-\rho(u_1^{(k+1)}-u_2^{(k+1)})=0,
\end{equation}                                                            
...
\label{alg:ADMM}
    ...
    $u_3^{(0)}=0$.\\
    2. Select $\rho\in(0,+\infty)$.\\
    3. For $k\in\mathbb{N}$, update $u_1$ by solving the linear system
    \begin{equation}
    \label{u1}
        (\frac{1}{\tau}-\epsilon^2\Delta_h+\rho)u_1^{(k+1)}=\frac{1}{\tau}u^n+\rho u_2^{(k)}-u_3^{(k)},
    \end{equation}
    \hspace{10pt} for $u_1^{(k+1)}\in\ch$ with PCG or FFT.\\
    4. Update $u_2$ by solving the nonlinear system
    \begin{equation}
    \label{eq:u2}
    \log(u_2^{(k+1)})-\log(1-u_2^{(k+1)})+
    \theta(1-2u^n)-u_3^{(k)}-\rho(u_1^{(k+1)}-u_2^{(k+1)})=0,
