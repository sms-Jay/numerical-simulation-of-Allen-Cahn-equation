...
try:
    energy_data = safe_read_data('energy_data.txt')
    if energy_data is not None:
        ...
        plt.tight_layout()
        plt.savefig('figures/energy_curve.png')
        plt.close()
...
try:
    ...
    admm_residual_data = safe_read_data('admm_residual_data.txt')
    ...
    if admm_residual_data is not None and admm_residual_data.shape[1] >= 2:
        ...
        plt.tight_layout()
        plt.savefig('figures/admm_convergence.png')
        plt.close()
...
try:
    bounds_data = safe_read_data('bounds_data.txt')
    if bounds_data is not None and bounds_data.shape[1] >= 3:
        ...
        plt.tight_layout()
        plt.savefig('figures/bounds_preservation.png')
        plt.close()
...
try:
    admm_data = safe_read_data('admm_iterations.txt')
    if admm_data is not None:
        ...
        plt.tight_layout()
        plt.savefig('figures/admm_iterations.png')
        plt.close()
