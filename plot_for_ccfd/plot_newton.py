...
def create_contour_plots_combined(u, x, y, time_indices):
    ...
    创建组合等高线图 - 多个时间步在一幅图中
    """
    output_dir = 'results/plot/newton'
    ...
    # 添加总标题
    fig.suptitle(f"The dynamical evolution of the phase variable computed by Newton's method", 
                 fontsize=14, fontweight='bold')
    
    plt.tight_layout(rect=[0, 0, 0.9, 0.95])  # 为颜色条和总标题留出空间
    plt.savefig(os.path.join(output_dir, "newton_evolution_0_to_50.png"), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"已生成组合图: newton_evolution_0_to_50.png")
...
def create_separate_plots(u, x, y, time_indices):
    ...
    for time in time_indices:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"newton_t_{time}.png"), dpi=300)
        plt.close()
        print(f"已生成单张图: newton_t_{time}.png")

def main():
    """主函数"""
    # 加载数据
    data_file = "H:/undergraduate/scientific_research/allen_cahn_equation_simulation/results/data/data_ccfd_newton_parallel.txt"
